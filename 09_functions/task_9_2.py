...
def generate_trunk_config(intf_vlan_mapping, trunk_template):
    ...
    for inf, vlans in intf_vlan_mapping.items():
        vlans_str = ",".join([str(i) for i in vlans])
        result.append(f"interface {inf}")
        for line in trunk_template:
            if line.endswith("allowed vlan"):
                result.append(f"switchport trunk allowed vlan {vlans_str}")
            else:
                result.append(line)
