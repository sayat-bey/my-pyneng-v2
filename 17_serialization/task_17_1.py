...
def write_dhcp_snooping_to_csv(filenames, output):
    ...
    for filename in filenames:
        hostname, *others = filename.split("_")
        with open(filename) as f:
            file = f.read()
            regex = r"(\S+) +(\S+) +\d+ +\S+ +(\d+) +(\S+)"
            
            for m in re.finditer(regex, file):
                result.append([hostname, m[1], m[2], m[3], m[4]])
...
write_dhcp_snooping_to_csv(["sw1_dhcp_snooping.txt", "sw2_dhcp_snooping.txt", "sw3_dhcp_snooping.txt"], "task_17_1_output.csv")
