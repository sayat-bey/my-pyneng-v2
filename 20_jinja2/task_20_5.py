...
def create_vpn_config(template1, template2, data_dict):
    file_dir,file1 = template1.split("/")
    *other,file2 = template2.split("/")
    
    env = Environment(loader=FileSystemLoader(file_dir), trim_blocks=True, lstrip_blocks=True)
    t1 = env.get_template(file1)
    t2 = env.get_template(file2)
    
    cfg1 = t1.render(data_dict)
    cfg2 = t2.render(data_dict)
