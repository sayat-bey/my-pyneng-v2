...
def convert_ranges_to_ip_list(ip_list):
    ...
    for ip in ip_list:
        if "-" not in ip:
            converted_ip.append(ip)
        else:
            ip1, ip2 = ip.split("-")
            
            oc1,oc2,oc3,*other = ip1.split(".")
            ip22 = f"{oc1}.{oc2}.{oc3}.{ip2.split('.')[-1]}"
            ipfirst = ipaddress.ip_address(ip1)
            iplast = ipaddress.ip_address(ip22)
            network = ipaddress.ip_network(f"{oc1}.{oc2}.{oc3}.0/24")
            
            for i in network:
                if ipfirst <= i <= iplast:
                    converted_ip.append(i)
